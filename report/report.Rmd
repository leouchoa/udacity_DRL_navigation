...
- the hyperparameters used: 
    - `BATCH_SIZE`: neural network mini-batch size
    - `GAMMA`: the discount factor used in the discounted sum of rewards
    - `TAU`: the $\tau$ parameter used to soft update of target parameters
    - `LR`: the neural network learning rate use in gradient descent
    - `UPDATE_EVERY`: how often to update the network
    - `BUFFER_SIZE`: replay buffer size (how much we store into the replay buffer)
